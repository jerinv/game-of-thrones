...
screenTime = pd.DataFrame({'Season':seasonNums,
                           'Episode':episodeNums,
                           'SceneStart':sceneStarts,
                           'SceneEnd':sceneEnds,
                           'Name':characterNames,})

screenTime['SceneStart'] = pd.to_timedelta(screenTime.SceneStart, unit='s')
screenTime['SceneEnd'] = pd.to_timedelta(screenTime.SceneEnd, unit='s')
screenTime['Duration'] = screenTime.SceneEnd - screenTime.SceneStart

total_screenTime_bySeason = screenTime.groupby(['Season','Name'])['Duration'].sum().reset_index()
...
df_plot = total_screenTime_bySeason[total_screenTime_bySeason.Name.isin(topNames)].copy()
df_plot['Duration_Hours'] = df_plot.Duration.apply(lambda x: x.total_seconds()/60)
df_plot = df_plot.pivot_table(index='Name', columns='Season', values='Duration_Hours', 
                              fill_value=0, 
                              aggfunc=np.sum,
                              margins=True)
...
ax.legend(loc=(0.000,0.01*topNum+0.57), 
          prop={'size':16}, 
          ncol=8, 
...
for i, duration in enumerate(topDurations):
    ax.text(duration+5, -i+(topNum-1)-.15, int(round(duration,0)), fontsize=16)

# Remove extra white space    
plt.subplots_adjust(top=0.985, right =0.99)

fig.savefig(r'images/got_screentime.png', facecolor=fig.get_facecolor(), bbox_inches='tight')
